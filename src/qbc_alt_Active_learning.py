...
import pandas as pd
from strategies import GaussianProcessBased, QueryByCommittee
...
qbc_active_learner = QueryByCommittee(random_state=seed)
...
for step in range(query_steps+1):
    ...
    if step == 0:
        ...
        X_train_df = pd.DataFrame(X_train, columns=Parameter_space.columns).astype(float)
        y_train_s = pd.Series(y_train[:, label_idx], dtype=float)

        query_idx_list = qbc_active_learner.query(X_unlabeled=X_pool_scaled, n_act=query_size, X_labeled=X_train_df, y_labeled=y_train_s)
...
with open(f'../result_single_pan/QBC_Alt_AL_PAN_results_seed_{seed}_{label_idx}.json', 'w') as f:
    json.dump(results, f)
