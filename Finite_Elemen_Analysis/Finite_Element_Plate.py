...
def print_load_and_reaction_summary(load_cases,nodes,p_global,p_global_post):
    ...
    for case in load_cases:
        ...
        results_lines.append(
            {
                'Load Case': case,
                'Case No': load_cases[case]['number'],
                'Case Type': load_cases[case]['case type'],
                'Applied Load (kN)': round(cases_applied_loads[case]['Fz']*10**-3,3),
                'Sum Reactions (kN)': round(cases_reactions[case]['Rz'],3)
            }
        )
